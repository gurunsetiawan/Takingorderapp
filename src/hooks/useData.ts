...
export function useData(authToken: string | null) {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const normalizedProducts: Product[] = (productsData.products || initialProducts).map((p: any) => ({
                id: p.id,
                name: p.name,
                code: p.code,
            ...
            const normalizedSales: Sale[] = (salesData.sales || []).map((s: any) => ({
                id: s.id,
            ...
            try {
                ...
                if (salesmenRes.ok) {
                    ...
                    const normalizedSalesmen: Salesman[] = (salesmenData.salesmen || []).map((s: any) => ({
                        ...s,
                ...
                console.log('Salesmen feature not available:', e);
                setSalesmen([]);
            }
            ...
            try {
                ...
                console.log('Locations feature not available:', e);
                setLocations([]);
            }
            ...
            try {
                ...
                if (customersRes.ok) {
                    ...
                    const normalizedCustomers: Customer[] = (customersData.customers || []).map((c: any) => ({
                        id: c.id,
                        code: c.code,
                        name: c.name,
                        phone: c.phone || '',
                ...
                console.log('Customers feature not available:', e);
                setCustomers([]);
            }
    ...
    // Salesman CRUD
    const addSalesman = async (salesman: Omit<Salesman, 'id'>): Promise<boolean> => {
        if (!authToken) return false;
        try {
            const newSalesman = { id: Date.now().toString(), ...salesman };
    ...
    const updateSalesman = async (id: string, salesman: Omit<Salesman, 'id'>): Promise<boolean> => {
        if (!authToken) return false;
        try {
    ...
    const deleteSalesman = async (id: string): Promise<boolean> => {
        if (!authToken) return false;
        try {
            const response = await fetch(`${API_URL}/salesmen/${id}`, {
                method: 'DELETE',
    ...
    // Location CRUD
    const addLocation = async (location: Omit<Location, 'id'>): Promise<boolean> => {
        if (!authToken) return false;
        try {
    ...
    const updateLocation = async (id: string, location: Omit<Location, 'id'>): Promise<boolean> => {
        if (!authToken) return false;
        try {
    ...
    // Customer CRUD
    const addCustomer = async (customer: Omit<Customer, 'id'>): Promise<boolean> => {
        if (!authToken) return false;
        try {
    ...
    const updateCustomer = async (id: string, customer: Omit<Customer, 'id'>): Promise<boolean> => {
        if (!authToken) return false;
        try {
